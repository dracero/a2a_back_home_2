...
class QdrantIndexerAgent:
    ...
    async def index_pdfs(self):
        """Indexa los PDFs en Qdrant, creando la colección si no existe."""
        # Primero procesamos un PDF para obtener el tamaño del vector
        ...
        # Intentar crear la colección primero
        try:
        ...
        try:
            ...
            self._save_chunks()
            return f"{'Colección creada y ' if created else ''}{len(points)} chunks indexados en Qdrant y guardados en memoria."
        except Exception as e:
...
class AgentFisic:
    ...
    async def _ensure_indexed(self):
        ...
        if not self.indexer_agent.chunkid_to_text:
            print("🔄 Verificando indexación de PDFs...")
